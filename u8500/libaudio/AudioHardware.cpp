...
** Lewa fix audio bug by ioz9 ,the last time,2012.03.28
...
static uint32_t SND_DEVICE_BT_EC_OFF=-1; // really DEVICE_BT_NSEC_OFF
static uint32_t SND_DEVICE_HEADSET=-1;
static uint32_t SND_DEVICE_HEADSET_AND_SPEAKER=-1; // really FORCE_SPEAKER
...
static uint32_t SND_DEVICE_FM_HEADSET=26; // really IN_S_SADC_OUT_HEADSET
    ...
    mFmRadioEnabled(false),mFmPrev(false),mFmVolume(0),fmfd(-1)
{
   if (get_audpp_filter() == 0) {
           LOGD("Lewa fix audio bug by ioz9 ,the last time,2012.03.28");
...
// removed by ioz9,beacause i want it like aurora
// #ifdef HAVE_FM_RADIO
/*
...
// copyed by ioz9,because it's very good in c8650
#ifdef HAVE_FM_RADIO
status_t AudioHardware::setFmOnOff(bool onoff)
{
    int ret;
    ...
    if (new_snd_device == -1) {
        ...
        if ((mTtyMode != TTY_OFF) && (mMode == AudioSystem::MODE_IN_CALL) &&
            ...
            if (outputDevices & AudioSystem::DEVICE_OUT_SPEAKER) {
                ...
                // from CM,Cooper's libaudio,by ioz9,2012-03-23
                new_post_proc_feature_mask = (ADRC_ENABLE | EQ_ENABLE | RX_IIR_ENABLE | MBADRC_ENABLE);
            }
#endif
        } else if (outputDevices & AudioSystem::DEVICE_OUT_WIRED_HEADSET) {
